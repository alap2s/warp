...
export const createWarp = async (data: { what: string; when: Date; where: string; icon: string; ownerId: string; type: 'public' | 'friends'; coordinates?: { lat: number; lng: number } | null }) => {
  try {
...
export const updateWarp = async (id: string, data: Partial<{ what: string; when: Date; where: string; icon: string; type: 'public' | 'friends'; coordinates?: { lat: number; lng: number } | null }>) => {
  try {
    const warpRef = doc(db, "warps", id);
